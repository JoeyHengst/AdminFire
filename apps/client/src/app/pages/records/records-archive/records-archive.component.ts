 ...
 */
@Component({
    selector: 'records-archive',
    templateUrl: './records-archive.component.html'
})
export class RecordsArchiveComponent {
    ...
    constructor(private afs: AngularFirestore, private db: FirestoreService) {
        this.records$ = this.db.col$('records', ref => ref.orderBy('createdAt').where('pending_removal', '==', false).where('archived', '==', true));
